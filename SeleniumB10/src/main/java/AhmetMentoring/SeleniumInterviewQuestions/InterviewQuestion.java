package AhmetMentoring.SeleniumInterviewQuestions;
...
public class InterviewQuestion {
      ...
      *Find Elements--> return the list of the elements
       ...
       *IsSelected -->the elements is selected or not.
        ...
        ********
        ...
        -I would use JavaScript with ScrollIntoView
        Point class (x, y coordinate)

        24) Selenium itself cannot verify file download, can click on download link but cannot go outside the browser and open the dwonload file.
        Selenium handles uploads but does it differently compared to actual user.
        *Find the element that triggers the upload window.
        driver.findElement(upload)

        Find the path of the file you want to ulpoad
        ...
        Assert: is a class that have some methods to validate teh actual and excpected data.
       The methods like equal, true, false we can do validation. (like comparison actual-expected results).

        26.What is the diffrence between validation and varification?


        What is the reason to have SoftAssert?

        It doesn't stop the code it will not give your exception.
        With SoftAssertAll you can find out which one is not working.

        With HardAssert Once you use Assert for validation if you code fails it will Right
        Away throws the exception which will stop the execution. (1000 and 1 example)

        Once you use SoftAssertion for validation, if code fails it will not throws exception Right Away and it will keep execution the codes.
         At the end you can see the failing ones to fix it.
         Do not forget to use SofAssert.All the end otherwise all test scenarious will pass.(might cause some issues)

         25. Validation execution the codes.
         Verification is reading the codes, no excecution.

         POM is the way to organize your implementation, test scenarious, specific methods
         (TestBase, Pages, Test, Utils) that help us to understand teh structure of the project.
 ...
 WebElement name=driver.findElement(By.id("id=\"ap_customer_name\""));
